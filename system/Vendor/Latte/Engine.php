...
use Latte\{Strict, Parser, Compiler, CompileException, Helpers, PhpHelpers, Loader};
use Latte\Macros\{CoreMacros, BlockMacros};
use Latte\Runtime\{FilterExecutor, Template};
/**
 * @package Latte Engine
 */
class Engine
{
	use Strict;
     ...
     */
    private string $cacheExtension = 'php';
	...
	public function __construct(array $config = [])
	{
        ...
        foreach ($config as $parameter => $value) :
            ...
            if (property_exists($this, $parameter)) $this->{$parameter} = $value;
	...
	public function createTemplate(string $name, array $params = []): Template
	{
        ...
        $engine = ClassManager::singleton(LatteEngine::class);
		return new $class($engine, $params, $this->filters, $this->providers, $name, $this->sandboxed ? $this->policy : null);
	}
	...
	private function isExpired(string $file, string $name): bool
	{
        ...
        $modifiedTime = file_exists($file) ? filemtime($file) : 0;
		return $this->autoRefresh && $this->getLoader()->isExpired($name, (int) $modifiedTime); // @ - file may not exist
	}
	...
	public function getCacheFile(string $name): string
	{
		$hash = substr($this->getTemplateClass($name), 8);
 		$base = preg_match('#([/\\\\][\w@.-]{3,35}){1,3}$#D', $name, $m)
			...
			: '';
		return "$this->tempDirectory/$base$hash." . $this->cacheExtension;
	}
	...
	{
		if (!$this->loader) {
			$this->loader = new FileLoader($this->baseDirectory);
		}
		return $this->loader;
	}
